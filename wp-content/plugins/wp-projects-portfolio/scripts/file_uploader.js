...
jQuery(document).ready(function() {
	...
	jQuery('#upload_cphoto_image_button').click(function() {
		jQuery('html').addClass('Image');
		
		var $cl_cont = jQuery(this).parent();
		$_clientphoto = jQuery('input[type=text]', $cl_cont);
		clformfield = $_clientphoto.attr('name');
	...
	window.send_to_editor = function(html){
		...
		if (clformfield) {
			...
			cl_fileurl = jQuery('img',html).attr('src');

			jQuery('#_clientphoto').val(cl_fileurl);
			
			$_clientphoto.val(cl_fileurl);
