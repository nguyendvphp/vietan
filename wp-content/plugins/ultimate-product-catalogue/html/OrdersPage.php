		...
		<div class="alignleft actions">
				<select name='action'>
  					<option value='-1' selected='selected'><?php _e("Bulk Actions", 'UPCP') ?></option>
						...
						<option value='hide'><?php _e("Hide", 'UPCP') ?></option>
				</select>
	...
	<thead>
		<tr>
			...
			<th scope='col' id='description' class='manage-column column-date  desc'  style="">
				...
				?>
				<span><?php _e("Order Date", 'UPCP') ?></span>
			</th>
		</tr>
	</thead>
	...
	<?php
		if ($Orders) { 
	  		foreach ($Orders as $Order) {
				...
				echo "<a class='delete-tag' href='admin.php?page=UPCP-options&Action=UPCP_DeleteOrder&DisplayPage=Orders&Order_ID=" . $Order->ID ."'>" . __("Delete", 'UPCP') . "</a>";
	 			echo "</span>";
				echo "</div>";
...
<div class="form-wrap">
<h3><?php _e("Add a New Order", 'UPCP') ?></h3>
...
<div class="form-field form-required">
	<label for="Order_Name"><?php _e("Name", 'UPCP') ?></label>
	<input name="Order_Name" id="Order_Name" type="text" value="" size="60" />
	<p><?php _e("The name of the order that will be displayed.", 'UPCP') ?></p>
</div>
<div class="form-field">
	<label for="Order_Description"><?php _e("Description", 'UPCP') ?></label>
	<textarea name="Order_Description" id="Order_Description" rows="5" cols="40"></textarea>
	<p><?php _e("The description of the order, to help with processing it.", 'UPCP') ?></p>
</div>
<div class="form-field form-required">
	<label for="Order_Items"><?php _e("Items", 'UPCP') ?></label>
	<input name="Order_Items" id="Order_Items" type="text" value="" size="60" />
	<p><?php _e("The items that were purchased in the order.", 'UPCP') ?></p>
</div>
<div class="form-field form-required">
	<label for="Order_Sales_Boolean"><?php _e("Sale?", 'UPCP') ?></label>
...
<p class="submit"><input type="submit" name="submit" id="submit" class="button-primary" value="<?php _e('Add New Order', 'UPCP') ?>"  /></p></form></div>
<br class="clear" />
</div>
</div>