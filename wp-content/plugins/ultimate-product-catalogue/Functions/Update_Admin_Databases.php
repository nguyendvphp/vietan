...
function Add_UPCP_Tag($Tag_Name, $Tag_Description, $Tag_Group_ID, $WC_Update = "No", $WC_term_id = 0) {
		...
		if ($WooCommerce_Sync == "Yes" and $WC_term_id != 0) {update_term_meta($WC_term_id, "upcp_ID", $wpdb->insert_id);}
		if ($WooCommerce_Sync == "Yes" and $WC_Update != "Yes") {UPCP_Add_Tag_To_WC($wpdb->get_row("SELECT * FROM $tags_table_name WHERE Tag_ID=%d", $wpdb->insert_id));}
...
function Add_UPCP_Product($Item_Name, $Item_Slug, $Item_Photo_URL, $Item_Description, $Item_Price, $Item_Sale_Price, $Item_Sale_Mode, $Item_SEO_Description, $Item_Link, $Item_Display_Status = "", $Category_ID = "", $Global_Item_ID = "", $Item_Special_Attr = "", $SubCategory_ID = "", $Tags = array(), $Related_Products = "", $Next_Previous = "", $Skip_Nonce = "No", $WC_ID = 0) {
		global $wpdb;
		global $items_table_name;
		global $categories_table_name;
		global $subcategories_table_name;
		...
		if (!wp_verify_nonce($_POST['_wpnonce']) and $Skip_Nonce != "Yes") {return __("There has been a validation error.", 'UPCP');}
		...
		if ($WooCommerce_Sync == "Yes" and get_option("UPCP_Product_Import") == "None") {
			$UPCP_Product = $wpdb->get_row($wpdb->prepare("SELECT * FROM $items_table_name WHERE Item_ID=%d", $Item_ID));
			update_option("UPCP_Product_Import", "UPCP");
...
function Update_UPCP_Options() {
	...
	if ($_POST['woocommerce_sync'] == "Yes" and get_option("UPCP_WooCommerce_Sync") == "No") {$Run_WC_Sync = "Yes";}
	else {$Run_WC_Sync = "No";}
